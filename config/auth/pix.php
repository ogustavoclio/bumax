			...
			<div class="col-9">
				<div class="containerBuild-noPadding">
					<div class="borderDown">
						<h4 style="margin: 0px !important;">Minha área Pix</h4>
					</div>
					<div class="cnt">
						<form>
							<h5 style="margin: 10px 0px -20px 10px">Tipo da chave:</h5><br>
							<select required name="tipoChave">
								<option value="0" <?php if($idPIX == 0) echo "selected"?>>Selecione...</option>
								<option value="1" <?php if($idPIX == 1) echo "selected"?>>CPF</option>
								<option value="2" <?php if($idPIX == 2) echo "selected"?>>CNPJ</option>
								<option value="3" <?php if($idPIX == 3) echo "selected"?>>E-mail</option>
								<option value="4" <?php if($idPIX == 4) echo "selected"?>>Telefone</option>
								<option value="5 " <?php if($idPIX == 5 ) echo "selected"?>>Chave Aleatória</option>
							</select><br>
							<h5 style="margin: 10px 0px -20px 10px">Chave PIX:</h5><br>
							<input required placeholder="Chave PIX..." <?php while($prod = mysqli_fetch_array($queryChavePix)) { ?><?php echo 'value="'; echo $prod["chavePix"]; if (empty($prod["chavePix"])) { echo '';} echo '"' ?><?php } ?> type="text" name="agencia"><br>
							<button style="margin: 20px 0px 0px 10px">Salvar</button>
						</form>
					</div>
				</div>
			</div>
